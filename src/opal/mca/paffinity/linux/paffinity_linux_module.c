 ...
 */
plpa_cpu_set_t global_paff_mask;
...
static int linux_module_init(void)
{
    ...
    plpa_have_topology_information(&supported);
    ...
    if (!supported) {
        ...
        plpa_sched_getaffinity(getpid(), sizeof(tmp), &tmp);
...
static int linux_module_set(opal_paffinity_base_cpu_set_t mask)
{
    ...
    if (0 != plpa_sched_setaffinity(getpid(), 
                                                         sizeof(plpa_mask), 
                                                         &plpa_mask)) {
...
static int linux_module_get(opal_paffinity_base_cpu_set_t *mask)
{
    ...
    if (0 != plpa_sched_getaffinity(getpid(), sizeof(plpa_mask), &plpa_mask)) {
        return OPAL_ERR_IN_ERRNO;
    }
    for (i = 0; i < 8 * sizeof(*mask); i++) {
...
static int linux_module_map_to_processor_id(int socket, int core, int *processor_id)
{
    int ret = plpa_map_to_processor_id(socket, core, 
                                                            processor_id);
    return convert(ret);
}
...
static int linux_module_map_to_socket_core(int processor_id, int *socket, int *core)
{
    int ret = plpa_map_to_socket_core(processor_id, 
                                                           socket, core);
    return convert(ret);
}
...
static int linux_module_get_processor_info(int *num_processors)
{
    ...
    int ret = plpa_get_processor_data(PLPA_COUNT_ONLINE,
                                                           num_processors, 
                                                           &max_processor_id);
...
static int linux_module_get_socket_info(int *num_sockets)
{
    ...
    int ret = plpa_get_socket_info(num_sockets, 
                                                        &max_socket_num);
    return convert(ret);
}
...
static int linux_module_get_core_info(int socket, int *num_cores)
{
    ...
    int ret = plpa_get_core_info(socket, num_cores, 
                                                      &max_core_num);
    return convert(ret);
}
...
static int get_physical_processor_id(int logical_processor_id)
{
    ...
    ret = plpa_get_processor_id(logical_processor_id,
                                                     PLPA_COUNT_ONLINE,
                                                     &phys_id);
    if (0 == ret) {
...
static int get_physical_socket_id(int logical_socket_id)
{
    ...
    ret = plpa_get_socket_id(logical_socket_id,
                                                  &phys_id);
    if (0 == ret) {
        return phys_id;
    } else {
...
static int get_physical_core_id(int physical_socket_id, int logical_core_id)
{
    ...
    ret = plpa_get_core_id(physical_socket_id,
